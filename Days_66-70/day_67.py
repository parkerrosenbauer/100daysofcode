...
# Upgraded blog website, allowing for creation, editing, and deletion of posts
# I didn't add all the html files because they've already been built/uploaded before, I want the focus to be on the work I did, which is this code here
...
def new_post():
    ...
    if form.validate_on_submit():
        post = BlogPost(title=form.title.data, subtitle=form.subtitle.data, date=datetime.now().date(),
                        body=form.body.data, author=form.author.data, img_url=form.img_url.data)
        db.session.add(post)
        db.session.commit()
        return redirect(url_for('get_all_posts'))
    return render_template("make-post.html", form=form, head="New")
...
def edit_post(post_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for('get_all_posts'))
    return render_template("make-post.html", form=form, head="Edit")
